{
	"translatorID": "b6e3ce77-8942-4d11-8259-342c49ae395f",
	"translatorType": 2,
	"label": "@BibLaTeX export bibliography to SMW",
	...
	function doExport() {
		...
		while (item = Zotero.nextItem()) {
			...
			if (!citekey) {
				...
				citekey = Zotero.BetterBibTeX.keymanager.get(item, 'on-export').citekey;
			}
			...
			if (item.date) {
				...
				writeField("date", Zotero.Utilities.strToDate(item.date).year);
			}
